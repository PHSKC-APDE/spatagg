...
stopifnot(all(hra$new %in% z2hra_fo[, target_id]))
stopifnot(all(hra$new %in% z2hra_pp[, target_id]))
...
library('mapview')
ud = hra %>% filter(new == 'U District')
udh = st_intersection(pp, summarize(ud))
udh = udh %>% mutate(case_when(is.na(pop) ~ 0, T ~ pop))
mapview(ud) + mapview(udh, zcol = 'pop')
setDT(z2hra_pp)
